...
class GinsengT5Regression(L.LightningModule):
    def __init__(self, args, fold, time_now):
        super().__init__()
        self.args = args
        self.fold = fold
        ...
        cols = [col for col in test_df.columns if col not in cols_to_remove]
        first_dim = len(cols)

        self.first_linear = nn.Linear(first_dim, args.hidden_dim)
    ...
    def training_step(self, batch, batch_idx):
        embed, adduct, target = batch
        pred = self(embed, adduct).squeeze(1)
        loss = self.loss(pred, target)

        pred_cpu = pred.detach().cpu()
        target_cpu = target.detach().cpu()
        self.log("train_loss", loss)
        self.log("train/r2_score", r2_score(target_cpu, pred_cpu))
        self.log("train/mse", mse(target_cpu, pred_cpu))
        self.log("train/mae", mae(target_cpu, pred_cpu))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/r2_score", r2_score(target_cpu, pred_cpu))
        self.log("val/mse", mse(target_cpu, pred_cpu))
        self.log("val/mae", mae(target_cpu, pred_cpu))
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test/r2_score", r2_score(target_cpu, pred_cpu))
        self.log("test/mse", mse(target_cpu, pred_cpu))
        self.log("test/mae", mae(target_cpu, pred_cpu))
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        pred = self(embed, adduct).squeeze(1)
        return {
            "gts": target.detach().cpu().tolist(),
            "preds": pred.detach().cpu().tolist(),
        }

    def configure_optimizers(self):
        optimizer = AdamW(self.parameters(), lr=self.args.lr)
        sch = lr_scheduler.StepLR(
...
class GinsengT5RegressionInference(L.LightningModule):
...
class RegressionPredictor:
    def __init__(self, args_with_adduct, args_without_adduct):
        ...
        self.model_with_adduct = GinsengT5RegressionInference.load_from_checkpoint(
            args_with_adduct.checkpoint_path, args=args_with_adduct
        )
        self.model_with_adduct.eval()

        self.model_without_adduct = GinsengT5RegressionInference.load_from_checkpoint(
            args_without_adduct.checkpoint_path, args=args_without_adduct
        )
